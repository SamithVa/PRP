...
const byte pumpOut = 24;

// ONE PUMP WATER OUT : DIGITAL
const byte pumpIn = 26;

// Actuator 
const byte actuatorSignal = 7; // PWM
...
const int TrackDeadZoneThreshold = 80; // Threshold for joystick being "in the center"
...
speed calibrateTrackSignal(int raw_x, int raw_y)
{
    ...
    // Forward
    else if (abs(x) < TrackDeadZoneThreshold && y > TrackDeadZoneThreshold) {
        ret.Left = trackMaxSpeed;
        ret.Right = trackMaxSpeed;
    }
    // Backward
    else if (abs(x) < TrackDeadZoneThreshold && y < -TrackDeadZoneThreshold){
        ret.Left = -trackMaxSpeed;
    ...
    // Forward-left
    else if (x < -TrackDeadZoneThreshold && y > TrackDeadZoneThreshold) {
        ret.Left = trackMaxSpeed * 0.6;
        ret.Right = trackMaxSpeed * 0.8;
    }
    // Forward-right
    else if (x > TrackDeadZoneThreshold && y > TrackDeadZoneThreshold) {
        ret.Left = trackMaxSpeed * 0.8;
        ret.Right = trackMaxSpeed * 0.6;
    }
     // Backward-left
    else if (x < -TrackDeadZoneThreshold && y < -TrackDeadZoneThreshold) {
        ret.Left = -trackMaxSpeed * 0.6;
    ...
    // Left only
    else if (x < -TrackDeadZoneThreshold && abs(y) < TrackDeadZoneThreshold) {
        // propeller.Left = trackMaxSpeed * 0.6;
        ret.Right = trackMaxSpeed * 0.8;
    }
    // Right only
    else if (x > TrackDeadZoneThreshold && abs(y) < TrackDeadZoneThreshold) {
        ret.Left = trackMaxSpeed * 0.8;
        // propeller.Right = trackMaxSpeed * 0.6;
    }
...
speed calibratePropellerSignal(int raw_x, int raw_y)
{
    ...
    // Forward
    else if (abs(x - 90) < SmallMovementThreshold && y > 90 + SmallMovementThreshold) {
        propeller.Left = 150;
        propeller.Right = 150;
    }
    // Backward
    ...
    // Left only
    else if (x < 90 - SmallMovementThreshold && abs(y - 90) < SmallMovementThreshold) {
        propeller.Left = 60;
        propeller.Right = 120;
    }
    // Right only
